	...
	<form class="cmp_form register" id="register" method="post" action="{url op="register"}">
			...
			<fieldset class="consent">
				{if $currentContext->getData('privacyStatement')}
					...
					<div class="form-check">
						<input class="form-check-input" type="checkbox" name="privacyConsent" id="privacyConsent" value="1"{if $privacyConsent} checked="checked"{/if}>
				...
				<div class="form-check">
					<input class="form-check-input" type="checkbox" name="emailConsent" id="emailConsent" value="1"{if $privacyConsent} checked="checked"{/if}>
					<label class="form-check-label" for="emailConsent">
						{translate key="user.register.form.emailConsent" privacyUrl=$privacyUrl}
					</label>
				</div>
			</fieldset>
		...
		{if !$currentContext}
			...
			{if $siteWidePrivacyStatement}
				...
				<div class="form-check">
					<input class="form-check-input" type="checkbox" name="privacyConsent[{$smarty.const.CONTEXT_ID_NONE}]" id="privacyConsent[{$smarty.const.CONTEXT_ID_NONE}]" value="1"{if $privacyConsent[$smarty.const.CONTEXT_ID_NONE]} checked="checked"{/if}>
					<label class="form-check-label" for="privacyConsent">
						{capture assign="privacyUrl"}{url router=$smarty.const.ROUTE_PAGE page="about" op="privacy"}{/capture}
						{translate key="user.register.form.privacyConsent" privacyUrl=$privacyUrl}
					</label>
			...
			{* Ask the user to opt into public email notifications *}
			<div class="form-check">
				<input class="form-check-input" type="checkbox" name="emailConsent" id="emailConsent" value="1"{if $emailConsent} checked="checked"{/if}>
				<label class="form-check-label" for="emailConsent">
		...
		<div class="buttons">
			...
			<div class="row">
				...
				<a class="btn btn-primary mw-100 mb-1" href="{url page="login" source=$rolesProfileUrl}" class="login">{translate key="user.login"}</a>				
			</div>
		</div>
	</form>
